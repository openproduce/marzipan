...
$sth = $dbh->prepare(qq{
		select balance from customers where id = 404
		});
$sth->execute();
...
$sth = $dbh->prepare(qq{
		select s.time_ended, si.quantity, i.name, si.total from sales as s, sale_items as si, inventory.items as i where s.id = si.sale_id and si.item_id = i.id and s.customer_id = 404 and payment = 4 order by s.time_ended desc
});
