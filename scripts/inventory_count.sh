...
echo 'select sum( if(i.count>0,  i.count*(di.wholesale_price/di.case_size),0)) from items as i, distributor_items as di, categories as c, category_items as ci where di.item_id = i.id and ci.cat_id = c.id and ci.item_id = i.id and (c.name = "spirits");' | mysql -u root inventory
